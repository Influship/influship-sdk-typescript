...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    description:
      'Fetch fresh Instagram post-page data for a bounded list of shortcodes. This returns one item per requested shortcode with per-item success or error details.\n\n**Note:** Batch post lookup is capped at 20 shortcodes per request and is charged for every requested shortcode.\n\n**Pricing**: 1 credit per post scraped ($0.01)',
    stainlessPath: '(resource) raw.instagram > (method) get_posts',
    qualified: 'client.raw.instagram.getPosts',
    params: ['shortcodes: string[];'],
    response:
      '{ data: { failed: number; items: { data: object; shortcode: string; success: true; } | { error: object; shortcode: string; status: number; success: false; }[]; requested: number; scraped_at: string; succeeded: number; }; }',
    markdown:
      "## get_posts\n\n`client.raw.instagram.getPosts(shortcodes: string[]): { data: object; }`\n\n**post** `/v1/raw/instagram/posts`\n\nFetch fresh Instagram post-page data for a bounded list of shortcodes. This returns one item per requested shortcode with per-item success or error details.\n\n**Note:** Batch post lookup is capped at 20 shortcodes per request and is charged for every requested shortcode.\n\n**Pricing**: 1 credit per post scraped ($0.01)\n\n### Parameters\n\n- `shortcodes: string[]`\n  Instagram post shortcodes from /p/, /reel/, or /tv/ URLs\n\n### Returns\n\n- `{ data: { failed: number; items: { data: object; shortcode: string; success: true; } | { error: object; shortcode: string; status: number; success: false; }[]; requested: number; scraped_at: string; succeeded: number; }; }`\n\n  - `data: { failed: number; items: { data: { post: { id: string; caption: string; comment_count: number; display_url: string; is_video: boolean; like_count: number; post_type: 'image' | 'video' | 'carousel'; shortcode: string; taken_at: number; accessibility_caption?: string; carousel_items?: object[]; coauthor_usernames?: string[]; display_resources?: object[]; engagement_visibility?: object; is_paid_partnership?: boolean; is_pinned?: boolean; location?: object; music_attribution?: object; owner_username?: string; product_mentions?: object[]; sponsor_usernames?: string[]; tagged_usernames?: string[]; thumbnail_url?: string; video_url?: string; video_versions?: object[]; view_count?: number; }; scraped_at: string; }; shortcode: string; success: true; } | { error: { error: 'not_found' | 'private' | 'rate_limited' | 'blocked' | 'invalid_input' | 'timeout' | 'unknown'; message: string; retry_after?: number; username?: string; }; shortcode: string; status: number; success: false; }[]; requested: number; scraped_at: string; succeeded: number; }`\n\n### Example\n\n```typescript\nimport Influship from 'influship';\n\nconst client = new Influship();\n\nconst response = await client.raw.instagram.getPosts({ shortcodes: ['C0ABC123xyz', 'D1DEF456uvw'] });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    description:
      'Transcribe an Instagram video post by shortcode and return the raw post-page data used for transcription. For now this raw endpoint retranscribes every request and piggybacks the post plus transcript into our database when the owner account exists.\n\n**Note:** Cached transcript reads are a planned follow-up; public pricing stays the same for live and cached transcript delivery.\n\n**Pricing**: 5 credits per transcript ($0.05)',
    ...
    response:
      "{ data: { full_text: string; language: string; post: { id: string; caption: string; comment_count: number; display_url: string; is_video: boolean; like_count: number; post_type: 'image' | 'video' | 'carousel'; shortcode: string; taken_at: number; accessibility_caption?: string; carousel_items?: object[]; coauthor_usernames?: string[]; display_resources?: object[]; engagement_visibility?: object; is_paid_partnership?: boolean; is_pinned?: boolean; location?: object; music_attribution?: object; owner_username?: string; product_mentions?: object[]; sponsor_usernames?: string[]; tagged_usernames?: string[]; thumbnail_url?: string; video_url?: string; video_versions?: object[]; view_count?: number; }; scraped_at: string; shortcode: string; source: 'native' | 'whisper' | 'groq'; transcript: string; word_count: number; duration_seconds?: number; model?: string; provider?: string; }; }",
    markdown:
      "## get_transcript\n\n`client.raw.instagram.getTranscript(shortcode: string, language?: string): { data: object; }`\n\n**get** `/v1/raw/instagram/transcript/{shortcode}`\n\nTranscribe an Instagram video post by shortcode and return the raw post-page data used for transcription. For now this raw endpoint retranscribes every request and piggybacks the post plus transcript into our database when the owner account exists.\n\n**Note:** Cached transcript reads are a planned follow-up; public pricing stays the same for live and cached transcript delivery.\n\n**Pricing**: 5 credits per transcript ($0.05)\n\n### Parameters\n\n- `shortcode: string`\n  Instagram post shortcode from a /p/, /reel/, or /tv/ URL\n\n- `language?: string`\n  Optional language code for transcription. Omit to auto-detect.\n\n### Returns\n\n- `{ data: { full_text: string; language: string; post: { id: string; caption: string; comment_count: number; display_url: string; is_video: boolean; like_count: number; post_type: 'image' | 'video' | 'carousel'; shortcode: string; taken_at: number; accessibility_caption?: string; carousel_items?: object[]; coauthor_usernames?: string[]; display_resources?: object[]; engagement_visibility?: object; is_paid_partnership?: boolean; is_pinned?: boolean; location?: object; music_attribution?: object; owner_username?: string; product_mentions?: object[]; sponsor_usernames?: string[]; tagged_usernames?: string[]; thumbnail_url?: string; video_url?: string; video_versions?: object[]; view_count?: number; }; scraped_at: string; shortcode: string; source: 'native' | 'whisper' | 'groq'; transcript: string; word_count: number; duration_seconds?: number; model?: string; provider?: string; }; }`\n\n  - `data: { full_text: string; language: string; post: { id: string; caption: string; comment_count: number; display_url: string; is_video: boolean; like_count: number; post_type: 'image' | 'video' | 'carousel'; shortcode: string; taken_at: number; accessibility_caption?: string; carousel_items?: { display_url: string; index: number; is_video: boolean; thumbnail_url?: string; video_url?: string; }[]; coauthor_usernames?: string[]; display_resources?: { config_height: number; config_width: number; src: string; }[]; engagement_visibility?: { comments_disabled?: boolean; like_and_view_counts_disabled?: boolean; viewer_can_reshare?: boolean; }; is_paid_partnership?: boolean; is_pinned?: boolean; location?: { id?: string; address_json?: object; has_public_page?: boolean; lat?: number; lng?: number; name?: string; slug?: string; }; music_attribution?: { artist_name?: string; audio_id?: string; should_mute_audio?: boolean; song_name?: string; uses_original_audio?: boolean; }; owner_username?: string; product_mentions?: { merchant_username?: string; product_id?: string; product_name?: string; }[]; sponsor_usernames?: string[]; tagged_usernames?: string[]; thumbnail_url?: string; video_url?: string; video_versions?: { url: string; id?: string; height?: number; type?: number; width?: number; }[]; view_count?: number; }; scraped_at: string; shortcode: string; source: 'native' | 'whisper' | 'groq'; transcript: string; word_count: number; duration_seconds?: number; model?: string; provider?: string; }`\n\n### Example\n\n```typescript\nimport Influship from 'influship';\n\nconst client = new Influship();\n\nconst response = await client.raw.instagram.getTranscript('C0ABC123xyz');\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    description:
      'Transcribe a bounded list of Instagram video posts by shortcode and return one item per requested shortcode with per-item success or error details. Successful items include the raw post-page data used for transcription.\n\n**Note:** Batch transcription is capped at 10 shortcodes per request, retranscribes every request for now, and is charged for every requested shortcode. Cached transcript reads are a planned follow-up; public pricing stays the same for live and cached transcript delivery.\n\n**Pricing**: 5 credits per transcript ($0.05)',
