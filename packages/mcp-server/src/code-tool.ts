...
const prompt = `Runs JavaScript code to interact with the Influship API.
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    headers: {
      ...
      client_envs: JSON.stringify({
        INFLUSHIP_API_KEY: requireValue(
          readEnv('INFLUSHIP_API_KEY') ?? client.apiKey,
          'set INFLUSHIP_API_KEY environment variable or provide apiKey client option',
        ),
        INFLUSHIP_BASE_URL: readEnv('INFLUSHIP_BASE_URL') ?? client.baseURL ?? undefined,
      }),
    },
    body: JSON.stringify({
      project_name: 'influship-api',
      code,
      intent,
      client_opts: {},
